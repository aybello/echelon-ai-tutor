...
- [ ] Merge and deploy approved PR #81 while preserving the current GPT-6 Astra Equipment Lab rebuild: integrate the code-only wastewater and 3D fallback changes, resolve visual-rebuild compatibility, run full validation and public production checks, and do not execute the math-article database correction or change question visibility. Production validation found that a successfully loaded page can remain on the Suspense loading panel while the lazy Three.js module itself is delayed; add timeout-based recovery at the page boundary before accepting release verification.
- [ ] Eliminate the release-blocking Class 1 Networks importer integration-test flake: isolate test fixtures from shared database state, ensure cleanup survives assertion failures/timeouts, and set an evidence-based timeout so full-suite validation is reliable without changing learner data.
