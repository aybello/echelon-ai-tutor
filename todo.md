...
- [ ] Complete the Echelon Equipment Lab clarifier release: current `/equipment-lab` has responsive 3D-style isometric/top/cutaway exploration, water/solids/flow controls, seven keyboard-operable components, operator and exam explanations, four process-flow stages, related-study navigation, a Resources entry, and reduced-motion-safe animation. Add the missing exploded-view experience, then publish and verify the live route and navigation entry on production before closing the release.
...
- [ ] Publish and production-verify the Equipment Lab: confirm the live `/equipment-lab` route, Resources navigation entry, all visual controls, and learner-safe behavior on the custom and managed production domains before marking the module complete.
...
- [ ] Repair the verified-email Team Flex learner flow: create/backfill a durable learner profile and auditable course-assignment record for eligible active Team Flex learners without changing existing access, preserve all existing login paths, and verify Thomas Johnson’s WPI Class IV Wastewater entitlement after the guarded migration.
- [ ] Verify whether the identified Winnipeg learner email currently has an authenticated learner-dashboard session path and active WPI Class IV Wastewater entitlement, using only read-only access and enrollment records.
