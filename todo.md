...
- [x] Correct the public OIT Water quiz heading, which still states `551+ Questions` after the clean launch restored 489 validated Water questions; replace hard-coded learner-facing inventory claims with the authoritative loaded bank metadata and add regression coverage — the heading and metadata now derive from `question_bank_meta.totalQuestions`, with neutral loading copy. Two dedicated regression tests, TypeScript, and production build passed.
