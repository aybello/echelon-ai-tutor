...
- [ ] Review, merge, and deploy Codex PRs #90 through #93 only after each change passes code review, required tests, TypeScript, production build, and GitHub Quality Gate checks.
- [ ] Replace PR #92’s direct Gemini API adapter with the platform-native server-side AI provider and an approved current native model, removing the `GEMINI_API_KEY` and `GEMINI_MODEL` release dependency; preserve bounded requests, cancellation, response validation, structured-output behavior, and all existing caller contracts before the combined PR #90–#93 release.
...
- [ ] Validate the new-student journey with controlled authentication and Stripe test-mode evidence before enabling live checkout; keep automated emails, managed jobs, and any historical-entitlement replay disabled until separately approved.
