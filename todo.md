...
- [ ] Repair, merge, and deploy authorized PR #87: use the exact current candidate head, correct the Class IV 100-scored/110-delivered denominator bug in learner result cards and recorded mock activity, add focused regression coverage, validate the full candidate, verify a recoverable production backup, apply only backup-gated additive migration 0064, verify its tables and migration ledger, publish, and production-verify the release. Do not activate the Class IV blueprint profile or perform unrelated question, learner-account, or content-data changes.
