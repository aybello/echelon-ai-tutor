...
- [ ] Deploy and production-verify the Three.js Equipment Lab enhancement on both custom and managed `/equipment-lab` routes: the release package adds a lazy-loaded 3D scene with orbit/top/cutaway/exploded views, seven-part selection, water/solids/flow controls, four clarification stages, keyboard-operable HTML component buttons, an ErrorBoundary/Canvas fallback, and the retained SVG Diagram view. Local validation passed: 1,335 tests, type checks, production build, public browser regression tests, and desktop/mobile visual reviews; production verification remains required.
