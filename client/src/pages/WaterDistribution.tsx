// Echelon — Water Distribution Process Guide
// Interactive step-by-step guide to the water distribution system
// Mirrors the structure of Process.tsx (drinking water treatment guide)

import { useState } from "react";
import { DIST_STEPS, DIST_LABEL_INFO, type DistributionStep } from "@/lib/distributionData";
...
function FlowMap({ active, onSelect }: { active: DistributionStep; onSelect: (s: DistributionStep) => void }) {
  return (
      ...
      <div style={{ display: "flex", alignItems: "center", minWidth: 900, padding: "4px" }}>
        {DIST_STEPS.map((s, i) => (
          <div key={s.id} style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>
            ...
            {i < DIST_STEPS.length - 1 && (
              <div style={{ flexShrink: 0, display: "flex", alignItems: "center" }}>
                <div style={{ width: 12, height: 2, background: "#E5E7EB" }} />
                <svg width={10} height={14} viewBox="0 0 10 14">
                  <polygon points="0,2 8,7 0,12" fill={active?.id === DIST_STEPS[i + 1]?.id ? "#0369A1" : "#CBD5E1"} />
                </svg>
              </div>
            )}
          </div>
...
export default function WaterDistributionGuide() {
  ...
  const [activeStep, setActiveStep] = useState<DistributionStep>(() => {
    const resumeId = getGuideResumeStep("water-distribution", DIST_STEPS.map((step) => step.id));
    return DIST_STEPS.find((step) => step.id === resumeId) ?? DIST_STEPS[0];
  });
  const [activeLabel, setActiveLabel] = useState<string | null>(null);
  const [view, setView] = useState<"learn" | "overview" | "map">(() => shouldResumeGuide() ? "learn" : "map");

  const labelDesc = activeLabel ? (DIST_LABEL_INFO[activeLabel] || null) : null;

  const handleLabelClick = (id: string | null) => {
    setActiveLabel(prev => prev === id ? null : id);
  };

  const handleStepSelect = (step: DistributionStep) => {
    setActiveStep(step);
    setActiveLabel(null);
  };

  const stepIdx = DIST_STEPS.findIndex(s => s.id === activeStep.id);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F1F5F9", fontFamily: "'Sora', sans-serif" }}>
      ...
      <SiteNav currentPath="/distribution-guide" />
      <GuideNav
        guideId="water-distribution"
        currentStepId={activeStep.id}
        currentStepLabel={activeStep.label}
        totalSteps={DIST_STEPS.length}
      />
      ...
      <div className="process-view-toggles" style={{ background: "#F8FAFC", borderBottom: "1px solid #E5E7EB", padding: "9px 28px", display: "flex", justifyContent: "center", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
        {([['map', '🗺️ Process Map'], ['learn', '🔬 Step Explorer'], ['overview', '📋 Full Overview']] as const).map(([v, l]) => (
          <button key={v} className="view-btn" onClick={() => setView(v)} style={{
            ...
            border: `1px solid ${view === v ? "#0369A1" : "#E5E7EB"}`,
            background: view === v ? "#E0F2FE" : "transparent",
            color: view === v ? "#0369A1" : "#64748B",
      ...
      <div className="process-main" style={{ padding: "24px 28px 60px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div className="process-flow-map-card" style={{
          ...
          <div className="process-flow-map-label" style={{ fontSize: 10, fontWeight: 700, color: "#94A3B8", letterSpacing: "0.1em", marginBottom: 14 }}>
            WATER DISTRIBUTION SYSTEM — CLICK ANY STEP TO EXPLORE
          </div>
          ...
          <div style={{ animation: "fadeUp 0.35s ease both" }}>
            <div style={{ background: "#fff", borderRadius: 16, padding: "28px 24px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", border: "1px solid #E5E7EB" }}>
              <div style={{ marginBottom: 18 }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: "#0F172A", marginBottom: 4 }}>Complete Water Distribution System</div>
                <div style={{ fontSize: 11, color: "#64748B" }}>All 7 stages from treatment plant to consumer tap — tap any step to open it in the Step Explorer</div>
              </div>
              <DistributionMap
                activeStepId={activeStep.id}
                onStepClick={(id) => {
                  const step = DIST_STEPS.find(s => s.id === id);
                  if (step) { handleStepSelect(step); setView("learn"); }
                }}
              />
              <div style={{ marginTop: 24, borderTop: "1px solid #F1F5F9", paddingTop: 20 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: "#94A3B8", letterSpacing: "0.1em", marginBottom: 12 }}>CHLORINE RESIDUAL JOURNEY THROUGH DISTRIBUTION</div>
                <div style={{ display: "flex", alignItems: "center", gap: 0, overflowX: "auto" }}>
                  {[
                    { label: "Pump Station", value: "0.5–2.0 mg/L", color: "#0369A1", bg: "#E0F2FE" },
                    { label: "Transmission", value: "0.3–1.5 mg/L", color: "#1D4ED8", bg: "#EFF6FF" },
                    { label: "Storage", value: "0.2–1.0 mg/L", color: "#7C3AED", bg: "#EDE9FE" },
                    { label: "Pressure Zones", value: "0.2–1.0 mg/L", color: "#059669", bg: "#D1FAE5" },
                    { label: "Dist. Mains", value: "0.1–0.8 mg/L", color: "#B45309", bg: "#FEF3C7" },
                    { label: "Service Conn.", value: "0.05–0.5 mg/L", color: "#DC2626", bg: "#FEE2E2" },
                    { label: "Consumer Tap", value: "≥0.05 mg/L ✓", color: "#059669", bg: "#D1FAE5" },
                  ].map((q, qi, arr) => (
          ...
          <div key={activeStep.id} style={{ animation: "fadeUp 0.35s ease both" }}>
            <div className="process-step-grid" style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 20 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                {/* Content authority */}
                <div style={{
                  background: "#E0F2FE", borderRadius: 14, padding: "16px 18px",
                  border: "1px solid #BAE6FD", borderLeft: "4px solid #0369A1",
                }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "#0369A1", letterSpacing: "0.1em", marginBottom: 6 }}>
                    EXAM GUIDANCE · ONTARIO CONTEXT
                  </div>
                  <div style={{ fontSize: 11, color: "#0C4A6E", lineHeight: 1.65 }}>{activeStep.regulation}</div>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  <button
                    onClick={() => stepIdx > 0 && handleStepSelect(DIST_STEPS[stepIdx - 1])}
                    disabled={stepIdx === 0}
                    style={{
                  ...
                  >← Previous</button>
                  <button
                    onClick={() => stepIdx < DIST_STEPS.length - 1 && handleStepSelect(DIST_STEPS[stepIdx + 1])}
                    disabled={stepIdx === DIST_STEPS.length - 1}
                    style={{
                      flex: 1, padding: "11px", borderRadius: 10, border: "none",
                      background: stepIdx === DIST_STEPS.length - 1 ? "#E5E7EB" : "linear-gradient(135deg,#0369A1,#0F766E)",
                      color: stepIdx === DIST_STEPS.length - 1 ? "#94A3B8" : "#fff",
                      fontSize: 12, fontWeight: 700,
                      cursor: stepIdx === DIST_STEPS.length - 1 ? "not-allowed" : "pointer",
          ...
          <div style={{ animation: "fadeUp 0.35s ease both" }}>
            <div style={{
              ...
              <div style={{ fontSize: 14, fontWeight: 800, color: "#0F172A", marginBottom: 22 }}>
                Complete Water Distribution System — WPI / Ontario
              </div>
              {DIST_STEPS.map((s, i) => (
                <div key={s.id}>
                  <div className="process-overview-row" style={{
                    ...
                    borderBottom: i < DIST_STEPS.length - 1 ? "1px solid #F1F5F9" : "none",
                  }}>
                    <div style={{
