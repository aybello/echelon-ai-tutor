// CLASS 1 WATER TREATMENT PRACTICE QUIZ
// 563-question bank · 15-question free trial · paid full access via PurchaseGate
// Mirrors OIT quiz (Home.tsx) structure
...
function toCompat(q: Class1WaterQuestion): QCompat {
...
export default function Class1WaterQuiz() {
  usePageMeta({
    title: "Class 1 Water Treatment Practice Quiz — 500+ Questions",
    description: "Practice for the Ontario Class 1 Water Treatment operator exam with 500+ questions across 8 modules. AI Tutor, step-by-step solutions, and confidence tracking included.",
    path: "/class1-water",
  ...
  const activeQuestions = useMemo<QCompat[]>(() => {
    const pool = selectedModule
      ? CLASS1_WATER_QUESTIONS.filter(q => q.module === selectedModule)
      : CLASS1_WATER_QUESTIONS;
  ...
  const handleModuleSelect = (mod: string | null) => {
    ...
    const pool = mod
      ? CLASS1_WATER_QUESTIONS.filter(q => q.module === mod).map(toCompat)
      : CLASS1_WATER_QUESTIONS.map(toCompat);
    setHistory([]);
  ...
  const moduleStats = useMemo(() => {
    ...
    CLASS1_WATER_QUESTIONS.forEach(q => {
        ...
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "48px 20px" }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: "40px 36px", boxShadow: "0 4px 24px rgba(0,0,0,0.08)", textAlign: "center" }}>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              ...
              <Link href="/class1-water-mock" style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", gap: 16, marginTop: 16, flexWrap: "wrap" }}>
            ...
            <button
              onClick={() => {
                ...
                const newPool = newCalcOnly ? CLASS1_WATER_QUESTIONS.filter(q => q.isCalc).map(toCompat) : CLASS1_WATER_QUESTIONS.map(toCompat);
                setHistory([]);
      ...
      {showGate && (
        <QuizGate
          ...
          paidFeatures={[
            ...
            "Class 1 Water Treatment Mock Exam (100 questions, 2-hour timer)",
