...
function buildExam(): WpiClass1WaterQuestion[] {
  const selected: WpiClass1WaterQuestion[] = [];
  for (const [module, target] of Object.entries(EXAM_MODULE_TARGETS)) {
    const pool = shuffle(wpiClass1WaterQuestions.filter((q) => q.module === module));
...
export default function WpiClass1WaterMockExam() {
  usePageMeta({
    title: "WPI Class I Water Treatment Mock Exam — 100 Questions",
    description:
      "Timed 100-question mock exam for the WPI Class I Water Treatment certification. BC EOCP Level I, Alberta AWWOA Class I, Saskatchewan, Manitoba. 2-hour timer, 70% pass threshold.",
    path: "/wpi-class1-water-mock",
    keywords:
  ...
  const [questions, setQuestions] = useState<WpiClass1WaterQuestion[]>([]);
  ...
  const results = useMemo(() => {
    ...
    for (const m of WPI_CLASS1_WATER_MODULES) byModule[m] = { correct: 0, total: 0 };
    answers.forEach((a, i) => {
      const q = questions[i];
      if (!q) return;
  ...
  if (examState === "intro") {
    return (
      ...
      >
        ...
        >
          <SiteNav currentPath="/wpi-class1-water-mock" />
