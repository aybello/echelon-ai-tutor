...
function Sparkline({ values, status }: { values: number[]; status: Telemetry["status"] }) {
  ...
  return (
    <svg viewBox="0 0 100 32" className="h-8 w-24" role="img" aria-label="Recent value trend">
      <polyline points={points} fill="none" stroke={colour} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
  ...
  return (
    <div className="relative grid place-items-center" style={{ width: size, height: size }}>
      ...
      <div className="text-center">
        ...
        <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Command score</div>
      </div>
    </div>
  );
}
...
export default function IncidentCommand() {
  ...
  if (mode === "intro") {
    return (
      <div className="min-h-screen bg-slate-950 text-white" style={{ fontFamily: "'Sora', sans-serif" }}>
        <SiteNav currentPath={location} brandName="Echelon Command" />
        <style>{`@keyframes commandPulse{0%,100%{opacity:.38;transform:scale(1)}50%{opacity:.9;transform:scale(1.08)}} .command-grid{background-image:linear-gradient(rgba(45,212,191,.05) 1px,transparent 1px),linear-gradient(90deg,rgba(45,212,191,.05) 1px,transparent 1px);background-size:32px 32px}`}</style>
        <main className="command-grid relative min-h-[calc(100vh-64px)] overflow-hidden">
          <div className="absolute left-[12%] top-24 h-72 w-72 rounded-full bg-blue-600/20 blur-[120px]" />
          <div className="absolute bottom-10 right-[8%] h-80 w-80 rounded-full bg-teal-500/15 blur-[130px]" />
              ...
              <div className="relative overflow-hidden rounded-2xl border border-slate-700 bg-slate-900/90 shadow-2xl">
                <div className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-xs font-black uppercase tracking-[.16em] text-slate-400">Cedar Ridge WTP</div>
                ...
                <div className="p-5">
                  <div className="mb-5 flex items-center justify-between text-xs text-slate-400"><span>Process overview</span><span>02:47:18</span></div>
        ...
        <main className="mx-auto max-w-7xl px-5 py-10">
          <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-slate-400">Cedar Ridge WTP, extreme-rain treatment-barrier event</p>
            </div>
          ...
          <div className="grid gap-5 lg:grid-cols-[340px_1fr]">
            <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                <div className="rounded-xl bg-slate-950 p-4"><div className="text-2xl font-black text-teal-300">{decisions.filter(d => d.points === 20).length}/5</div><div className="mt-1 text-[10px] uppercase tracking-wider text-slate-500">Optimal calls</div></div>
                <div className="rounded-xl bg-slate-950 p-4"><div className="text-2xl font-black text-blue-300">74 min</div><div className="mt-1 text-[10px] uppercase tracking-wider text-slate-500">Simulated time</div></div>
              </div>
          ...
          <section className="mt-6 overflow-hidden rounded-2xl border border-slate-800 bg-slate-900">
            <div className="border-b border-slate-800 px-6 py-4"><h2 className="font-black">Decision timeline</h2></div>
            <div className="divide-y divide-slate-800">
              {decisions.map((decision, index) => (
                <div key={decision.stepId} className="grid gap-3 px-6 py-5 md:grid-cols-[60px_1fr_110px] md:items-center">
                  <div className="text-xs font-black text-slate-500">{SCENARIO_STEPS[index].time}</div>
                  <div><div className="text-sm font-bold text-white">{decision.choiceLabel}</div><div className="mt-1 text-xs leading-5 text-slate-400">{decision.consequence}</div></div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white" style={{ fontFamily: "'Sora', sans-serif" }}>
      ...
      <main className="mx-auto max-w-[1500px] px-4 py-5">
        <header className="mb-4 flex flex-col justify-between gap-4 rounded-2xl border border-slate-800 bg-slate-900 px-5 py-4 lg:flex-row lg:items-center">
          ...
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-5 items-start gap-2">
                {PROCESS_NODES.map((node, index) => {
                  ...
                  return <div key={node} className="relative flex flex-col items-center text-center">
                    ...
                    <div className={`relative z-10 grid h-12 w-12 place-items-center rounded-full border-2 transition-all ${active ? "border-rose-400 bg-rose-400/20 text-rose-200 shadow-[0_0_25px_rgba(251,113,133,.25)]" : completed ? "border-teal-400 bg-teal-400/15 text-teal-300" : "border-slate-700 bg-slate-950 text-slate-500"}`}>{active ? <Activity className="h-5 w-5" /> : index === 0 ? <Waves className="h-5 w-5" /> : index === 4 ? <Droplets className="h-5 w-5" /> : <Gauge className="h-5 w-5" />}</div>
                    <span className={`mt-3 text-[10px] font-bold ${active ? "text-white" : "text-slate-500"}`}>{node}</span>
                  </div>;
                })}
              </div>
            </section>
            ...
            <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              {step.telemetry.map(item => (
                <div key={item.label} className={`rounded-2xl border bg-slate-900 p-4 ${item.status === "critical" ? "border-rose-400/40" : item.status === "warning" ? "border-amber-400/30" : "border-slate-800"}`}>
                  <div className="flex items-start justify-between gap-2"><div className="text-[10px] font-bold text-slate-500">{item.label}</div><span className={`mt-1 h-2 w-2 rounded-full ${item.status === "critical" ? "animate-pulse bg-rose-400" : item.status === "warning" ? "bg-amber-400" : "bg-teal-400"}`} /></div>
                  <div className="mt-3 flex items-end justify-between gap-2"><div><span className={`text-2xl font-black ${item.status === "critical" ? "text-rose-300" : item.status === "warning" ? "text-amber-300" : "text-white"}`}>{item.value}</span><span className="ml-1 text-[10px] text-slate-500">{item.unit}</span></div><Sparkline values={item.trend} status={item.status} /></div>
                </div>
              ))}
            </section>
            ...
            <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
              <div className="mb-4 flex items-start gap-4">
                ...
                <div><div className="text-[10px] font-black uppercase tracking-[.18em] text-blue-300">Command decision {stepIndex + 1}</div><h2 className="mt-1 text-xl font-black">{step.title}</h2><p className="mt-2 text-sm leading-6 text-slate-400">{step.briefing}</p></div>
              </div>
              ...
              <div className="space-y-3">
                {step.choices.map((choice, index) => {
                  ...
                  return <button key={choice.id} disabled={Boolean(selectedChoice)} onClick={() => choose(choice)} className={`group w-full rounded-xl border p-4 text-left transition ${chosen ? choice.points === 20 ? "border-teal-400 bg-teal-400/10" : choice.points > 0 ? "border-amber-400 bg-amber-400/10" : "border-rose-400 bg-rose-400/10" : disabled ? "border-slate-800 bg-slate-950/50 opacity-40" : "border-slate-700 bg-slate-950/70 hover:border-blue-400 hover:bg-blue-400/[.05]"}`}>
                    <div className="flex items-start gap-3"><div className={`grid h-7 w-7 shrink-0 place-items-center rounded-lg text-xs font-black ${chosen ? "bg-white text-slate-950" : "bg-slate-800 text-slate-400 group-hover:bg-blue-500 group-hover:text-white"}`}>{chosen ? choice.points === 20 ? <CheckCircle2 className="h-4 w-4" /> : <XCircle className="h-4 w-4" /> : String.fromCharCode(65 + index)}</div><div><div className="text-sm font-bold leading-6 text-slate-100">{choice.label}</div><div className="mt-1 text-xs leading-5 text-slate-500">{choice.rationale}</div></div></div>
                  </button>;
                })}
              </div>
              {selectedChoice && <div className={`mt-4 rounded-xl border p-4 ${selectedChoice.points === 20 ? "border-teal-400/30 bg-teal-400/[.06]" : selectedChoice.points > 0 ? "border-amber-400/30 bg-amber-400/[.06]" : "border-rose-400/30 bg-rose-400/[.06]"}`}><div className="flex items-start justify-between gap-4"><div><div className="text-[10px] font-black uppercase tracking-[.16em] text-slate-400">Plant consequence</div><p className="mt-2 text-sm leading-6 text-slate-200">{selectedChoice.consequence}</p></div><span className={`shrink-0 rounded-full px-3 py-1 text-xs font-black ${selectedChoice.points === 20 ? "bg-teal-400/15 text-teal-200" : selectedChoice.points > 0 ? "bg-amber-400/15 text-amber-200" : "bg-rose-400/15 text-rose-200"}`}>+{selectedChoice.points}</span></div><button onClick={continueScenario} disabled={debriefMutation.isPending} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-black text-slate-950 transition hover:bg-slate-200 disabled:opacity-60">{debriefMutation.isPending ? "Generating GPT-5.6 review..." : stepIndex === SCENARIO_STEPS.length - 1 ? "Generate after-action review" : "Advance incident"}<ChevronRight className="h-4 w-4" /></button></div>}
            </section>
          </div>

          <aside className="space-y-4">
            <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
              <div className="mb-4 flex items-center justify-between"><h2 className="text-sm font-black">Live plant state</h2><span className="text-[10px] text-slate-500">consequence model</span></div>
              <div className="space-y-4">
                {plantState.map(item => {
                  ...
                  return <div key={item.label}><div className="mb-2 flex items-center justify-between text-[11px]"><span className="font-bold text-slate-400">{item.label}</span><span className={`font-black ${textColour}`}>{item.value}%</span></div><div className="h-1.5 overflow-hidden rounded-full bg-slate-800"><div className={`h-full rounded-full transition-all duration-500 ${colour}`} style={{ width: `${item.value}%` }} /></div></div>;
                })}
              </div>
              <p className="mt-4 text-[10px] leading-5 text-slate-500">Every decision changes treatment-barrier protection, available operating capacity or the defensibility of the incident record.</p>
            </section>
            <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
              <div className="mb-4 flex items-center justify-between"><h2 className="text-sm font-black">Incident timeline</h2><span className="text-[10px] text-slate-500">live record</span></div>
              <div className="space-y-4">
                <div className="flex gap-3"><div className="flex flex-col items-center"><span className="mt-1 h-2.5 w-2.5 rounded-full bg-rose-400" /><span className="mt-1 h-full w-px bg-slate-800" /></div><div className="pb-2"><div className="text-[10px] font-black text-slate-500">02:14</div><div className="mt-1 text-xs font-bold text-slate-200">Extreme rainfall response initiated</div></div></div>
                {decisions.map((decision, index) => <div key={decision.stepId} className="flex gap-3"><div className="flex flex-col items-center"><span className={`mt-1 h-2.5 w-2.5 rounded-full ${decision.points === 20 ? "bg-teal-400" : decision.points > 0 ? "bg-amber-400" : "bg-rose-400"}`} /><span className="mt-1 h-full w-px bg-slate-800" /></div><div className="pb-2"><div className="text-[10px] font-black text-slate-500">{SCENARIO_STEPS[index].time}</div><div className="mt-1 text-xs font-bold leading-5 text-slate-200">{decision.choiceLabel}</div><div className="mt-1 line-clamp-2 text-[10px] leading-4 text-slate-500">{decision.consequence}</div></div></div>)}
            ...
            <section className="rounded-2xl border border-blue-400/20 bg-blue-400/[.05] p-5">
              ...
              <p className="mt-3 text-xs leading-6 text-slate-400">GPT-5.6 will evaluate the complete chain of decisions, including what you prioritized, what you verified and how your choices changed the plant.</p>
            </section>
            <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
              ...
              <p className="mt-2 text-[11px] leading-5 text-slate-500">This simulator teaches decision structure. Real operators must follow their facility emergency plan, approved procedures and governing requirements.</p>
            </section>
          </aside>
        </div>
      </main>
