// WPI Class 2 Water Distribution Quiz — Unified via useQuizSession hook
...
const MODULE_COLORS: Record<string, { bg: string; color: string }> = {
  "Advanced Distribution Systems": { bg: "#DBEAFE", color: "#1D4ED8" },
  "System Design & Engineering": { bg: "#DCFCE7", color: "#15803D" },
  "Water Quality Management": { bg: "#EDE9FE", color: "#6D28D9" },
  "Regulatory Compliance": { bg: "#CCFBF1", color: "#0F766E" },
  "Advanced Calculations": { bg: "#FFEDD5", color: "#C2410C" },
};
...
export default function WpiClass2WaterDistQuiz() {
  ...
  const { questions: dbQuestions, modules: dbModules, overviews: dbOverviews, isLoading: bankLoading, dbUnavailable } = useQuestionBank("wpi-class2-water-dist", "lazy");
  ...
  const session = useQuizSession({ examType: "wpi-class2-water-dist", allQuestions });
  ...
  return (
    <QuizShell
      ...
      courseLabel="WPI Class 2 · Water Distribution"
      courseTitle="WPI Class 2 Water Distribution Quiz"
      ...
      gate={session.trialDone && !session.trialUnlocked ? (
        <QuizGate
          ...
          productName="WPI Class 2 Water Distribution Practice Pass"
          priceLabel="CA$99"
          paidFeatures={[
            "500 WPI Class 2 Water Distribution questions — unlimited attempts",
