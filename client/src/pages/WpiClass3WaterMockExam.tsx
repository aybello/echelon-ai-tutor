...
function buildExam(): WpiClass3WaterQuestion[] {
  const selected: WpiClass3WaterQuestion[] = [];
  for (const [module, target] of Object.entries(EXAM_MODULE_TARGETS)) {
    const pool = shuffle(wpiClass3WaterQuestions.filter((q) => q.module === module));
...
export default function WpiClass3WaterMockExam() {
  usePageMeta({
    ...
    description:
      "Timed 100-question mock exam for the WPI Class III Water Treatment certification. BC EOCP Level I, Alberta AWWOA Class I, Saskatchewan, Manitoba. 2-hour timer, 70% pass threshold.",
    path: "/wpi-class3-water-mock",
    keywords:
  ...
  const [questions, setQuestions] = useState<WpiClass3WaterQuestion[]>([]);
  ...
  const results = useMemo(() => {
    ...
    for (const m of WPI_CLASS3_WATER_MODULES) byModule[m] = { correct: 0, total: 0 };
    answers.forEach((a, i) => {
      const q = questions[i];
      if (!q) return;
  ...
  if (examState === "intro") {
    return (
      ...
      backPath="/wpi"
        ...
        price={79}
        features={[
          "531 WPI Class III questions — unlimited practice",
      ...
      >
        ...
        >
          <SiteNav currentPath="/wpi-class3-water-mock" />
          <div style={{ maxWidth: 680, margin: "0 auto", padding: "64px 24px" }}>
            ...
            <div style={{ marginTop: 24 }}>
              <ScoreHistory examType="wpi-class3-water" sessionId="wpi-class3-water" />
            </div>
          </div>
        </div>
      </PurchaseGate>
      ...
      >
        ...
        <div style={{ maxWidth: 820, margin: "0 auto", padding: "40px 24px 80px" }}>
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {WPI_CLASS3_WATER_MODULES.filter((module) => (results.byModule[module]?.total ?? 0) > 0).map((module) => {
