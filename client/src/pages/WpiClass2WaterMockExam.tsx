...
// WPI Class II exam blueprint: proportional module targets
...
function buildExam(): WpiClass2WaterQuestion[] {
  const selected: WpiClass2WaterQuestion[] = [];
  for (const [module, target] of Object.entries(EXAM_MODULE_TARGETS)) {
    const pool = shuffle(wpiClass2WaterQuestions.filter((q) => q.module === module));
...
export default function WpiClass2WaterMockExam() {
  usePageMeta({
    ...
    keywords:
      "WPI Class II mock exam, BC EOCP Level I exam, Alberta AWWOA Class I exam, water treatment mock exam, WPI practice exam",
  });

  const [examState, setExamState] = useState<ExamState>("intro");
  const [questions, setQuestions] = useState<WpiClass2WaterQuestion[]>([]);
  ...
  const results = useMemo(() => {
    ...
    for (const m of WPI_CLASS2_WATER_MODULES) byModule[m] = { correct: 0, total: 0 };
    answers.forEach((a, i) => {
      const q = questions[i];
      if (!q) return;
  ...
  if (examState === "intro") {
    return (
      ...
      backPath="/wpi"
        ...
        price={79}
        features={[
          "598 WPI Class II questions — unlimited practice",
      ...
      >
        ...
        >
          <SiteNav currentPath="/wpi-class2-water-mock" />
          <div style={{ maxWidth: 680, margin: "0 auto", padding: "64px 24px" }}>
            ...
            <div style={{ marginTop: 24 }}>
              <ScoreHistory examType="wpi-class2-water" sessionId="wpi-class2-water" />
            </div>
          </div>
        </div>
      </PurchaseGate>
      ...
      >
        ...
        <div style={{ maxWidth: 820, margin: "0 auto", padding: "40px 24px 80px" }}>
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {WPI_CLASS2_WATER_MODULES.filter((module) => (results.byModule[module]?.total ?? 0) > 0).map((module) => {
