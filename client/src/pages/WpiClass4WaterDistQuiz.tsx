// WPI Class 4 Water Distribution Quiz — Unified via useQuizSession hook
...
const MODULE_COLORS: Record<string, { bg: string; color: string }> = {
  "Expert Distribution Systems": { bg: "#DBEAFE", color: "#1D4ED8" },
  "Expert System Design": { bg: "#DCFCE7", color: "#15803D" },
  "Research & Water Quality": { bg: "#EDE9FE", color: "#6D28D9" },
  "Executive Compliance": { bg: "#CCFBF1", color: "#0F766E" },
  "Advanced Engineering Math": { bg: "#FFEDD5", color: "#C2410C" },
};
const MODULE_ICONS: Record<string, string> = {
  "Expert Distribution Systems": "🔧",
...
export default function WpiClass4WaterDistQuiz() {
  usePageMeta({
    title: "WPI Class 4 Water Distribution Practice Questions",
    description: "Practice Questions for WPI Class 4 Water Distribution operator certification exam. Practice with hundreds of questions aligned to Canadian provincial standards.",
  });

  const { questions: dbQuestions, modules: dbModules, overviews: dbOverviews, formulaLinks, isLoading: bankLoading, dbUnavailable } = useQuestionBank("wpi-class4-water-dist", "lazy");
  ...
  const session = useQuizSession({ examType: "wpi-class4-water-dist", allQuestions });
    ...
    <QuizShell
      ...
      gate={session.trialDone && !session.trialUnlocked ? (
        <QuizGate
          ...
          productName="WPI Class 4 Water Distribution Practice Pass"
          priceLabel="CA$99"
          paidFeatures={[
            "500 WPI Class 4 Water Distribution questions — unlimited attempts",
