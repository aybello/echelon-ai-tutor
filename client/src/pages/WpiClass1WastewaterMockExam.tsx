...
function buildExam(): WpiClass1WastewaterQuestion[] {
  const selected: WpiClass1WastewaterQuestion[] = [];
  for (const [module, target] of Object.entries(EXAM_MODULE_TARGETS)) {
    const pool = shuffle(wpiClass1WastewaterQuestions.filter((q) => q.module === module));
...
export default function WpiClass1WastewaterMockExam() {
  usePageMeta({
    title: "WPI Class I Wastewater Treatment Mock Exam — 100 Questions",
    description:
      "Timed 100-question mock exam for the WPI Class I Wastewater Treatment certification. BC EOCP Level I, Alberta AWWOA Class I, Saskatchewan, Manitoba. 2-hour timer, 70% pass threshold.",
    path: "/wpi-class1-wastewater-mock",
    keywords:
  ...
  const [questions, setQuestions] = useState<WpiClass1WastewaterQuestion[]>([]);
  ...
  function submitExam() {
    ...
    saveScore.mutate({
      ...
      sessionId: "wpi-class1-wastewater",
      stream: "water",
    });
    setExamState("results");
  }
  ...
  const results = useMemo(() => {
    ...
    for (const m of WPI_CLASS1_WASTEWATER_MODULES) byModule[m] = { correct: 0, total: 0 };
    answers.forEach((a, i) => {
      const q = questions[i];
      if (!q) return;
  ...
  if (examState === "intro") {
    return (
      ...
      >
        ...
        >
          <SiteNav currentPath="/wpi-class1-wastewater-mock" />
          <div style={{ maxWidth: 680, margin: "0 auto", padding: "64px 24px" }}>
            ...
            <div style={{ marginTop: 24 }}>
              <ScoreHistory examType="wpi-class1-wastewater" sessionId="wpi-class1-wastewater" />
            </div>
          </div>
        </div>
      </PurchaseGate>
      ...
      >
        ...
        >
          <div style={{ color: "#fff", fontSize: 14, fontWeight: 700 }}>
            WPI Class I Wastewater Treatment Mock Exam
          </div>
      ...
      >
        ...
        <div style={{ maxWidth: 820, margin: "0 auto", padding: "40px 24px 80px" }}>
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {WPI_CLASS1_WASTEWATER_MODULES.filter((module) => (results.byModule[module]?.total ?? 0) > 0).map((module) => {
