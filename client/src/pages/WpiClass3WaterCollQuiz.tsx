// WPI Class 3 Wastewater Collection Quiz — Unified via useQuizSession hook
...
const MODULE_COLORS: Record<string, { bg: string; color: string }> = {
  "Complex Collection Systems": { bg: "#DBEAFE", color: "#1D4ED8" },
  "Advanced System Design": { bg: "#DCFCE7", color: "#15803D" },
  "Leadership & Compliance": { bg: "#EDE9FE", color: "#6D28D9" },
  "Expert Calculations": { bg: "#CCFBF1", color: "#0F766E" },
  "Advanced Environmental": { bg: "#FFEDD5", color: "#C2410C" },
};
...
export default function WpiClass3WaterCollQuiz() {
  usePageMeta({
    title: "WPI Class 3 Water Collection Practice Questions",
    description: "Practice Questions for WPI Class 3 Water Collection operator certification exam. Practice with hundreds of questions aligned to Canadian provincial standards.",
    noindex: true
  });

  const { questions: dbQuestions, modules: dbModules, overviews: dbOverviews, formulaLinks, isLoading: bankLoading, dbUnavailable } = useQuestionBank("wpi-class3-wastewater-coll", "lazy");
  ...
  const session = useQuizSession({ examType: "wpi-class3-water-coll", allQuestions });
  ...
  return (
    <QuizShell
      ...
      courseLabel="WPI Class 3 · Wastewater Collection"
      courseTitle="WPI Class 3 Wastewater Collection Quiz"
      courseSubtitle="Canada & US · ABC/WPI Standard · Class III"
      ...
      gate={session.trialDone && !session.trialUnlocked ? (
        <QuizGate
          ...
          productName="WPI Class 3 Wastewater Collection Practice Pass"
          priceLabel="CA$199"
          paidFeatures={[
            "500 WPI Class 3 Wastewater Collection questions — unlimited attempts",
