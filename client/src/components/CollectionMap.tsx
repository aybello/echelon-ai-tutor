...
const PIPE_LABELS: Record<string, string> = {
  "sources→lateral":          "Building\ndrain",
  "lateral→gravity_sewer":    "Min 2%\nslope",
  "gravity_sewer→manholes":   "Gravity\nflow",
  "manholes→liftstation":     "Collected\nsewage",
  "liftstation→forcemain":    "Pumped\nunder pressure",
  "forcemain→cso":            "Force main\ndischarge",
};
...
export default function CollectionMap({ onStepClick, activeStepId }: CollectionMapProps) {
  ...
  return (
    <div style={{ width: "100%", overflowX: "auto", overflowY: "visible" }}>
      <style>{`
        @keyframes collFlowDash { from { stroke-dashoffset: 20 } to { stroke-dashoffset: 0 } }
        .coll-flow-pipe { animation: collFlowDash 0.8s linear infinite; }
        .coll-step-node { cursor: pointer; transition: filter 0.15s; }
        .coll-step-node:hover { filter: brightness(1.08) drop-shadow(0 4px 12px rgba(0,0,0,0.18)); }
      `}</style>
      <svg
      ...
      >
        <defs>
          <marker id="coll-arrow" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
          ...
          return (
            <g key={i}>
              ...
              <path
                ...
                className="coll-flow-pipe" markerEnd="url(#coll-arrow)" opacity={0.7}
              />
              {c.label && (
                <g>
                  <rect x={c.midX - 36} y={c.midY - 14} width={72} height={28} rx={6} fill="white" stroke="#E2E8F0" strokeWidth={1} />
                  {c.label.split("\n").map((line, li) => (
        ...
        {steps.map((step, i) => {
          ...
          return (
            <g key={step.id} className="coll-step-node" onClick={() => onStepClick(step.id)} transform={`translate(${x}, ${y})`}>
              {isActive && (
