  ...
  return (
    <div style={{ width: "100%", overflowX: "auto", WebkitOverflowScrolling: "touch" as const }}>
      <style>{`
        ...
        .dist-snake-node { cursor: pointer; }
      `}</style>
      <svg viewBox={`0 0 ${totalW} ${totalH}`} style={{ width: "100%", minWidth: 560, display: "block" }} xmlns="http://www.w3.org/2000/svg">
        <defs>
          {connectors.map((c) => (
            <marker key={c.id} id={`dist-arr-${c.id}`} markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
        ...
        {connectors.map((c) => (
          <g key={c.id}>
            ...
            <path d={c.d} stroke={c.color} strokeWidth={4} fill="none" strokeLinecap="round" strokeLinejoin="round"
              strokeDasharray="9 6" className="dist-snake-pipe" markerEnd={`url(#dist-arr-${c.id})`} opacity={0.82} />
            {c.label && (
              <g>
        ...
        {steps.map((step, i) => {
          ...
          return (
            <g key={step.id} className="dist-snake-node" onClick={() => onStepClick(step.id)} transform={`translate(${x},${y})`}>
