...
export function RadialGauge({
  ...
  const statusColor = status === "critical" ? "#F43F5E" : status === "warning" ? "#F59E0B" : "#2DD4BF";
  const statusGlow = status === "critical" ? "rgba(244,63,94,.4)" : status === "warning" ? "rgba(245,158,11,.3)" : "rgba(45,212,191,.2)";
    ...
    <div className="flex flex-col items-center">
      <svg width={size} height={size * 0.65} viewBox={`0 0 ${size} ${size * 0.65}`}>
        ...
        <path
          ...
          style={{ filter: `drop-shadow(0 0 6px ${statusGlow})`, transition: "stroke-dashoffset 0.8s cubic-bezier(0.23,1,0.32,1)" }}
        />
        {/* Needle */}
        ...
        {/* Center value */}
        <text x={size / 2} y={size * 0.6 - 8} textAnchor="middle" fill="white" fontSize={size * 0.16} fontWeight={900} fontFamily="'JetBrains Mono', monospace">
          {displayValue.toFixed(1)}
        </text>
        <text x={size / 2} y={size * 0.6 + 8} textAnchor="middle" fill="#94A3B8" fontSize={9} fontWeight={600}>
          {unit}
        </text>
      </svg>
      <span className="mt-1 text-[9px] font-bold uppercase tracking-[.14em] text-slate-400">{label}</span>
    </div>
  );
}
...
export function AlarmPanel({ alarms }: { alarms: Alarm[] }) {
  ...
  const severityConfig = {
    critical: { bg: "bg-rose-500/15", border: "border-rose-400/50", text: "text-rose-300", dot: "bg-rose-400", icon: AlertTriangle },
    warning: { bg: "bg-amber-500/10", border: "border-amber-400/40", text: "text-amber-300", dot: "bg-amber-400", icon: Zap },
    info: { bg: "bg-blue-500/10", border: "border-blue-400/30", text: "text-blue-300", dot: "bg-blue-400", icon: Activity },
  };
    ...
    <div className="rounded-xl border border-slate-700 bg-[#0B1120] p-3">
      <div className="mb-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className={`h-2 w-2 rounded-full ${alarms.some(a => a.severity === "critical") ? (flashState ? "bg-rose-400" : "bg-transparent") : "bg-teal-400"}`} />
          <span className="text-[9px] font-black uppercase tracking-[.16em] text-slate-400">Alarm register</span>
        </div>
        <span className="text-[9px] font-mono text-slate-500">{alarms.length} active</span>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[8px] font-mono text-slate-500">{alarm.time}</div>
              </div>
            </div>
          );
        })}
  ...
  return (
    <div className="rounded-xl border border-slate-700 bg-[#0B1120] p-4">
      <div className="mb-3 flex items-center justify-between">
        <span className="text-[9px] font-black uppercase tracking-[.16em] text-slate-400">Process topology</span>
        <div className="flex items-center gap-1.5 text-[8px] text-teal-300">
      ...
      <svg viewBox="0 0 600 80" className="w-full" style={{ minHeight: 60 }}>
        ...
        {nodes.map((_, i) => {
          ...
          return (
            <line
              ...
              x2={x2 - 16}
              y2={40}
              stroke={completed ? "#2DD4BF" : "#334155"}
              strokeWidth={2}
        ...
        {nodes.map((node, i) => {
          ...
          const fillColor = active ? "#F43F5E" : completed ? (failed ? "#F59E0B" : "#2DD4BF") : "#1E293B";
          const strokeColor = active ? "#FB7185" : completed ? (failed ? "#FBBF24" : "#5EEAD4") : "#475569";
          const textColor = active ? "#FFF" : completed ? "#E2E8F0" : "#94A3B8";

          return (
            <g key={node}>
              {/* Glow for active */}
              {active && <circle cx={x} cy={40} r={22} fill="none" stroke="#F43F5E" strokeWidth={1} opacity={0.4} filter="url(#scadaGlow)">
                <animate attributeName="r" values="20;24;20" dur="1.5s" repeatCount="indefinite" />
                <animate attributeName="opacity" values="0.4;0.1;0.4" dur="1.5s" repeatCount="indefinite" />
              </circle>}
              <circle cx={x} cy={40} r={16} fill={fillColor} stroke={strokeColor} strokeWidth={2} opacity={0.9} />
              {/* Icon indicator */}
...
export function TelemetryTicker({ items }: { items: TelemetryItem[] }) {
    ...
    const interval = setInterval(() => {
      setFluctuated(prev =>
        prev.map((item, i) => {
          ...
          // Small random fluctuation ±2% of value
  ...
  // Sync when items change (new step)
  useEffect(() => {
    setFluctuated(items);
  }, [items]);
  ...
  return (
    <div className="grid grid-cols-2 gap-2">
      {fluctuated.map((item) => {
        ...
        const statusColor = item.status === "critical" ? "text-rose-300" : item.status === "warning" ? "text-amber-300" : "text-teal-300";
        const borderColor = item.status === "critical" ? "border-rose-500/40" : item.status === "warning" ? "border-amber-400/30" : "border-slate-700";
        const bgColor = item.status === "critical" ? "bg-rose-500/[.06]" : item.status === "warning" ? "bg-amber-400/[.04]" : "bg-[#0B1120]";

        return (
          <div key={item.label} className={`rounded-lg border ${borderColor} ${bgColor} p-2.5`}>
            <div className="flex items-center justify-between">
              <Icon className={`h-3 w-3 ${statusColor}`} />
              <span className={`h-1.5 w-1.5 rounded-full ${item.status === "critical" ? "animate-pulse bg-rose-400" : item.status === "warning" ? "bg-amber-400" : "bg-teal-400"}`} />
            </div>
            <div className="mt-2">
              ...
              <span className="ml-1 text-[8px] text-slate-500">{item.unit}</span>
            </div>
...
function MiniSparkline({ values, status }: { values: number[]; status: "normal" | "warning" | "critical" }) {
  ...
  const color = status === "critical" ? "#F43F5E" : status === "warning" ? "#F59E0B" : "#2DD4BF";
...
// ─── LIVE PLANT STATE BARS (enhanced) ─────────────────────────────────────────
export function PlantStateBars({ items }: { items: { label: string; value: number }[] }) {
  return (
    <div className="rounded-xl border border-slate-700 bg-[#0B1120] p-3">
      <div className="mb-2 flex items-center justify-between">
        <span className="text-[9px] font-black uppercase tracking-[.16em] text-slate-400">Barrier integrity</span>
        <span className="text-[8px] font-mono text-slate-500">real-time</span>
      </div>
      <div className="space-y-2.5">
        {items.map(item => {
          const color = item.value >= 75 ? "#2DD4BF" : item.value >= 50 ? "#F59E0B" : "#F43F5E";
          const glow = item.value >= 75 ? "rgba(45,212,191,.2)" : item.value >= 50 ? "rgba(245,158,11,.2)" : "rgba(244,63,94,.2)";
          return (
            <div key={item.label}>
              ...
              <div className="h-1.5 overflow-hidden rounded-full bg-slate-800">
                <div
                  ...
                  style={{ width: `${item.value}%`, background: color, boxShadow: `0 0 8px ${glow}` }}
                />
              </div>
            </div>
          );
...
export function ScadaHeader({
  ...
  return (
    <header className="mb-3 rounded-xl border border-slate-700 bg-[#0B1120] px-4 py-3">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="grid h-8 w-8 place-items-center rounded-lg border border-slate-700 bg-slate-800 text-slate-300 transition hover:border-rose-400/40 hover:text-white">
          ...
          <div className="flex items-center gap-3">
            <div className="grid h-9 w-9 place-items-center rounded-lg bg-rose-500/15">
              <AlertTriangle className="h-4 w-4 text-rose-400" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-sm font-black text-white">{facilityName}</span>
                <span className="rounded-full bg-rose-500/20 px-2 py-0.5 text-[8px] font-black uppercase tracking-wider text-rose-300">
                  INCIDENT ACTIVE
                </span>
              </div>
              <span className="text-[10px] text-slate-400">{incidentLabel}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-5 text-[10px]">
          ...
          <div className="text-center">
            ...
            <div className="mt-0.5 font-mono text-base font-black tabular-nums text-white">{clockDisplay}</div>
          </div>
          {/* Step */}
          <div className="text-center">
            <div className="text-[8px] font-bold uppercase tracking-wider text-slate-500">STEP</div>
            <div className="mt-0.5 font-mono text-base font-black tabular-nums text-white">{stepIndex + 1}/{totalSteps}</div>
          </div>
          {/* Score gauge */}
          <div className="text-center">
            <div className="text-[8px] font-bold uppercase tracking-wider text-slate-500">SCORE</div>
            <div className="mt-0.5 flex items-center gap-2">
              <div className="h-1.5 w-20 overflow-hidden rounded-full bg-slate-800">
                <div className="h-full rounded-full bg-teal-400 transition-all duration-500" style={{ width: `${commandScore}%`, boxShadow: "0 0 8px rgba(45,212,191,.3)" }} />
              </div>
              <span className="font-mono text-sm font-black text-teal-300">{commandScore}</span>
            </div>
          </div>
        </div>
      </div>
