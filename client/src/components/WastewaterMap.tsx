...
const PIPE_LABELS: Record<string, string> = {
  "screening→primary":      "Raw sewage\n200–300 mg/L BOD",
  "primary→aeration":       "Primary effluent\n~150 mg/L BOD",
  "aeration→secondary":     "Activated sludge\nmix",
  "secondary→disinfection": "Secondary effluent\n20–30 mg/L BOD",
  "disinfection→tertiary":  "Disinfected\neffluent",
  "tertiary→discharge":     "Treated\n<10 mg/L BOD",
};
...
export default function WastewaterMap({ onStepClick, activeStepId }: WastewaterMapProps) {
  ...
  return (
    <div style={{ width: "100%", overflowX: "auto", overflowY: "visible" }}>
      <style>{`
        @keyframes wwFlowDash { from { stroke-dashoffset: 20 } to { stroke-dashoffset: 0 } }
        .ww-flow-pipe { animation: wwFlowDash 0.8s linear infinite; }
        .ww-step-node { cursor: pointer; transition: filter 0.15s; }
        .ww-step-node:hover { filter: brightness(1.08) drop-shadow(0 4px 12px rgba(0,0,0,0.18)); }
      `}</style>
      <svg
      ...
      >
        <defs>
          <marker id="ww-arrow" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
        ...
        {steps.map((step, i) => {
          ...
          return (
            <g key={step.id} className="ww-step-node" onClick={() => onStepClick(step.id)} transform={`translate(${x}, ${y})`}>
              {isActive && (
