...
export default function WastewaterMap({ onStepClick, activeStepId }: WastewaterMapProps) {
  ...
  return (
    <div style={{ width: "100%", overflowX: "auto", overflowY: "visible" }}>
      <style>{`
        @keyframes wwFlow { from { stroke-dashoffset: 24 } to { stroke-dashoffset: 0 } }
        .ww-pipe { animation: wwFlow 0.7s linear infinite; }
        .ww-node { cursor: pointer; }
      `}</style>
      <svg
      ...
      >
        <defs>
          {steps.map((s) => (
            <marker key={s.id} id={`wwarr-${s.id}`} markerWidth="10" markerHeight="10" refX="9" refY="5" orient="auto">
        ...
        {steps.slice(0, -1).map((s, i) => {
          ...
          return (
            <g key={s.id}>
              ...
              <line x1={fromX} y1={cy} x2={toX - 12} y2={cy} stroke={s.color} strokeWidth={5} strokeLinecap="round"
                strokeDasharray="10 7" className="ww-pipe" markerEnd={`url(#wwarr-${s.id})`} opacity={0.85} />
              {label && (
                <g>
        ...
        {steps.map((step, i) => {
          ...
          return (
            <g key={step.id} className="ww-node" onClick={() => onStepClick(step.id)} transform={`translate(${x}, ${y})`}>
