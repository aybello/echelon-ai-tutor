...
These are facilitated courses. Public reading can be self-directed, but the planned hours include source reading, calculations, written practical work, instructor discussion and assessment. Reading length alone does not substantiate those hours. Each module contains a minute-by-minute activity allocation in its curriculum record. Final assessment and evaluation time is included in the last module, not added to the advertised total.

For each ten-hour flagship, plan two five-hour learning days excluding breaks. Day one covers modules 1–3; day two covers modules 4–6. Each module is 100 minutes. Schedule suitable breaks outside those contact minutes. The four-module focused courses use 45- or 60-minute modules, with breaks outside the 180- or 240-minute total.

Before launch, a qualified instructor must run a timed pilot with representative learners. Record actual active time separately for instruction, source reading, practical work, discussion, assessment and breaks. Record where learners needed additional explanation, whether the final allocation was sufficient, and whether learning outcomes were achieved. Revise duration and activities from this evidence; do not pad a course to preserve a planned number. A shorter validated duration requires corresponding catalogue and approval-package changes.
...
Give learners the prerequisites and delivery schedule before enrollment. Check access to a calculator, the source readings and an accessible means of submitting written work. Provide an equivalent accessible exercise or reasonable timing accommodation where needed, documenting the arrangement without lowering the learning standard. Platform controls use native labels and keyboard-operable inputs; an accessibility review with actual learners remains part of the pilot.
...
A passing final alone is insufficient. Completion also requires accepted practicals, instructor-verified participation for the validated course duration, and a course evaluation. Attendance records must identify completed session dates, contact minutes excluding breaks, instructor and qualification evidence, plus attendance/engagement support. Page-open time is not attendance evidence. The application prevents more than seven hours within one course on one day; instructors must also check the learner's combined schedule across courses and external activities. Do not double-count overlapping sessions.
