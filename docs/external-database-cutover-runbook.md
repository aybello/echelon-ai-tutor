...
6. Restart the application, then run a live smoke check for the home page, sign-in, one existing learner entitlement, pricing availability, and a read-only organization dashboard query. Confirm that the public cutover status has returned to normal mode and that the protected internal health check reports a connected database before reopening customer-facing writes. Do not create a live Stripe payment for this check.
