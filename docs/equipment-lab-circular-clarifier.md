...
## Resilient 3D fallback follow-up — 2026-09-13.pr81-trust-repair.1

The PR #81 follow-up adds bounded recovery for the deferred Three.js module. If the module download is unavailable or remains unresolved for 12 seconds, the page automatically disables 3D mode and opens the preserved interactive Diagram view with a visible status notice. It also handles unavailable WebGL and a lost active context. This is a reliability safeguard; it does not change the educational content, component set, data model, or conceptual boundary.

Checkpoint `f1a3ebbb` published the initial PR #81 code release and checkpoint `734e6288` published the timeout recovery plus test-fixture reliability repair. Full local validation and the seven-case public browser suite passed on both managed and custom domains, covering a ready 3D scene or explicit Diagram fallback, reduced motion, desktop/mobile no-WebGL, lost context, failed module loading, and delayed module recovery. No authentication, learner action, answer submission, or learner-data mutation was used during verification.
