...
3. Apply **only `drizzle/0062_purchase_email_outbox.sql`** through the controlled release process. Do not run the unrestricted `pnpm db:migrate:apply` command: it applies all pending migrations. Use the backup-gated `pnpm db:migrate:apply-standalone` command with `MIGRATION_TARGET=0062_purchase_email_outbox`; it rejects out-of-scope targets and verifies the outbox table and indexes before it marks the ledger row applied. Unrelated pending migrations must remain untouched. This migration only creates the outbox table; it does not update historical purchases.
