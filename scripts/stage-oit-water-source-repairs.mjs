...
import { analyseQuestion } from "/home/ubuntu/echelon-ai-tutor/server/answerLengthBias.ts";
import { closeDatabaseConnection } from "/home/ubuntu/echelon-ai-tutor/server/connectionCleanup.ts";
...
const requestedQuestionNumbers = questionNumbersArgument
  ? new Set(
      questionNumbersArgument
        .slice("--question-nums=".length)
        .split(",")
...
const allCandidates = JSON.parse(
  await readFile(
    "/home/ubuntu/echelon-ai-tutor/docs/oit-water-source-repair-candidates-2026-08-30.json",
    "utf8",
  ),
);
const approvedQuestionNumbers = approvedOnly
  ? new Set(
      JSON.parse(await readFile("/home/ubuntu/echelon-ai-tutor/docs/oit-water-source-repair-gpt-independent-review-2026-08-30.json", "utf8")).reviews
        .filter(review => review.approved === true)
        .map(review => review.questionNum),
    )
  : null;
const approvedCandidates = approvedQuestionNumbers
  ? allCandidates.filter(candidate => approvedQuestionNumbers.has(candidate.questionNum))
  : allCandidates;
const candidates = requestedQuestionNumbers
  ? approvedCandidates.filter(candidate => requestedQuestionNumbers.has(candidate.questionNum))
  : approvedCandidates;
...
if (!Array.isArray(allCandidates) || allCandidates.length !== 15) {
  throw new Error("Expected exactly 15 controlled source-repair candidates.");
}
if (approvedOnly && candidates.length === 0) throw new Error("Independent review did not approve any candidates.");
if (requestedQuestionNumbers && candidates.length !== requestedQuestionNumbers.size) {
  throw new Error("One or more requested question numbers is not an approved controlled candidate.");
}
...
if (validateOnly) {
  console.log(JSON.stringify({ status: "validation_passed", candidateCount: candidates.length }, null, 2));
  process.exit(0);
}
...
try {
  ...
  if (rows.length !== candidates.length) {
    throw new Error(`Expected ${candidates.length} existing rows but found ${rows.length}.`);
  }
  const q104 = rows.find(row => row.questionNum === 104);
  if (questionNumbers.includes(104) && q104?.reviewStatus !== "in_review") {
    throw new Error("q104 must remain in_review before staging source repair.");
  }
  const unexpectedStatuses = rows.filter(row => row.questionNum !== 104 && !["unreviewed", "in_review"].includes(row.reviewStatus));
  if (unexpectedStatuses.length > 0) {
    throw new Error(`Unexpected review status: ${JSON.stringify(unexpectedStatuses)}`);
  }
