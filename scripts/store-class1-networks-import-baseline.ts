...
const baselinePath = process.env.CLASS1_NETWORKS_IMPORT_BASELINE_PATH ?? "/home/ubuntu/class1_networks_import_release/class1-networks-import-baseline.json";
const evidencePath = path.join(path.dirname(baselinePath), "class1-networks-import-baseline.evidence.json");
...
if (!baseline.baselineChecksum || !baseline.packageChecksum || !baseline.questionSnapshotChecksum || !baseline.metadataSnapshotChecksum || !baseline.attemptSnapshotChecksum) {
  throw new Error("Class 1 network import rollback baseline is incomplete.");
}
const storageKey = `governed-content-backups/class1/networks-import/${baseline.baselineChecksum}.json`;
const { key, url } = await storagePut(storageKey, bytes, "application/json");
const evidence = {
  schemaVersion: 1,
  capturedAtUtc: new Date().toISOString(),
  purpose: "Managed rollback evidence for the guarded Class 1 Distribution and Collection additive import.",
  snapshotChecksum,
