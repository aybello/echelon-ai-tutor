...
add("Water Treatment", "Disinfection", "EPA_WATER", "chlorine demand", "describe the chlorine consumed before a measurable residual remains", "A 2.0 mg/L dose produces a 0.6 mg/L residual", "recognize that 1.4 mg/L was consumed by chlorine demand", "Chlorine dose equals chlorine demand plus the measured residual", "dose and residual measurements reconcile within expected process variation");
...
add("Water Treatment", "Water Chemistry", "EPA_WATER", "alkalinity", "provide acid-neutralizing capacity and help stabilize treatment pH", "Alum addition causes a larger-than-normal pH drop", "check raw-water alkalinity and apply approved alkalinity adjustment if required", "Low alkalinity water has limited buffering capacity against acid-producing reactions", "pH remains more stable during normal coagulant dosing");
...
add("Wastewater Treatment", "Disinfection", "EPA_WW", "dechlorination", "remove excess chlorine residual before discharge where required", "Chlorinated effluent meets bacteria goals but residual is too high", "verify and adjust the approved dechlorinating chemical feed", "Dechlorination protects receiving waters but overdosing can consume oxygen or add chemicals", "final residual meets the discharge target without excessive chemical use");
...
add("Wastewater Collection", "Wet-Weather Flow", "EPA_COLL", "inflow", "describe direct stormwater entry through sources such as roof drains or open covers", "Flow spikes almost immediately when rainfall begins", "inspect rapid inflow sources before focusing on slow groundwater leakage", "Inflow responds quickly to rainfall because it enters through direct connections or openings", "removing a verified direct source reduces the immediate wet-weather spike");
add("Wastewater Collection", "Wet-Weather Flow", "EPA_COLL", "infiltration", "describe groundwater entering through defects in buried pipes, joints and structures", "Elevated sewer flow continues for days after rain stops", "inspect groundwater-related defects using flow, CCTV or testing data", "Infiltration often has a delayed and sustained response compared with direct inflow", "repairing submerged defects reduces prolonged post-storm base flow");
...
function numericDistractors(value, scale = 1, decimals = 1) {
  const candidates = [value * 0.8, value * 1.2, value + scale, Math.max(0, value - scale), value * 1.5]
    .map(v => fmt(v, decimals))
    .filter(v => v !== value);
  return [...new Set(candidates)].slice(0, 3);
}
function addCalc(stream, module, topic, stem, correctValue, unit, formula, work, source, decimals = 1, scale = 1) {
  const value = fmt(correctValue, decimals);
  const distractors = numericDistractors(value, scale, decimals).map(v => `${v} ${unit}`.trim());
  addQuestion({
...
for (const [influent, effluent] of wtCases.removal) addCalc("Water Treatment","Math & Calculations","turbidity removal",`Raw-water turbidity is ${influent} NTU and settled-water turbidity is ${effluent} NTU. What is the removal efficiency?`,(influent-effluent)/influent*100,"%","Removal = (influent - effluent) / influent x 100",`(${influent} - ${effluent}) / ${influent} x 100`,"EPA_FILTER",1,5);
...
for (const [influent,effluent] of wwtCases.removal) addCalc("Wastewater Treatment","Math & Calculations","BOD removal",`Influent BOD is ${influent} mg/L and effluent BOD is ${effluent} mg/L. What is the BOD removal efficiency?`,(influent-effluent)/influent*100,"%","Removal = (influent - effluent) / influent x 100",`(${influent} - ${effluent}) / ${influent} x 100`,"EPA_WW",1,5);
