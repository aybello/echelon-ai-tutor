...
const bankKeys = Object.freeze({ water: "class1-water", wastewater: "class1-wastewater" });
...
function validateBank(name, questions) {
  ...
  if (questions.filter((question) => question.isCalc === "yes").length !== 50) errors.push("Expected 50 calculation questions.");
  questions.forEach((question, index) => {
    const label = `${name}#${question.questionNum}`;
    if (question.bankKey !== bankKeys[name] || Number(question.questionNum) !== 2001 + index) errors.push(`${label}: identity mismatch.`);
    const stem = normalise(question.question);
    if (stems.has(stem)) errors.push(`${label}: duplicate stem.`);
    stems.add(stem);
    if (!Array.isArray(question.options) || question.options.length !== 4 || new Set(question.options.map((value) => value.trim().toLowerCase())).size !== 4) errors.push(`${label}: invalid options.`);
    if (question.correctIndex < 0 || question.correctIndex > 3 || question.correctAnswer !== question.options?.[question.correctIndex]) errors.push(`${label}: answer mismatch.`);
    else answerPositions[question.correctIndex] += 1;
    for (const field of ["module", "question", "explanation", "sourceTitle", "sourceReference", "sourceUrl", "blueprintObjective"]) {
      if (!question[field]) errors.push(`${label}: missing ${field}.`);
    }
    if (!["recall", "application"].includes(question.cognitiveLevel)) errors.push(`${label}: invalid cognitive level.`);
    if (question.reviewStatus !== "unreviewed") errors.push(`${label}: candidate review status must be unreviewed.`);
    for (let optionIndex = 0; optionIndex < 4; optionIndex += 1) {
      if (question[`option${"ABCD"[optionIndex]}`] !== question.options?.[optionIndex]) errors.push(`${label}: option-field mismatch.`);
    }
  });
  if (Math.max(...answerPositions) - Math.min(...answerPositions) > 10) errors.push("Answer positions are materially unbalanced.");
  const answerCues = analyseOitAnswerCues(questions);
  if (answerCues.longTells.length || answerCues.shortTells.length || answerCues.qualifierTells.length || answerCues.longestRate > 0.4 || answerCues.shortestRate > 0.5) errors.push("Answer-cue review required.");
  return { valid: errors.length === 0, errors, answerPositions, answerCues };
}

export const CLASS1_NETWORKS_BASELINE_COUNTS = Object.freeze({
  [bankKeys.water]: 555,
  [bankKeys.wastewater]: 565,
});
...
const packageNames = Object.freeze({
  [bankKeys.water]: "water",
  [bankKeys.wastewater]: "wastewater",
});
...
export function loadClass1NetworksPackage(repoRoot = path.resolve(import.meta.dirname, "../..")) {
  const contentRoot = path.join(repoRoot, "content", "class1-treatment");
  const manifestPath = path.join(contentRoot, "manifest.json");
  const manifestBytes = fs.readFileSync(manifestPath);
  const manifest = JSON.parse(manifestBytes.toString("utf8"));
  if (manifest.status !== "independently-cleared-release-candidate") {
...
export function createBaseline({ checksum, rows, metadata, attemptRows, capturedAtUtc = new Date().toISOString() }) {
  ...
  const payload = {
    schemaVersion: 1,
    capturedAtUtc,
    purpose: "Pre-import full-bank rollback snapshot for the Class 1 Water Treatment and Wastewater Treatment additive release.",
...
export async function stageClass1NetworksPackage({ connection, payloads, checksum, baseline, questionsTable = "questions", metaTable = "question_bank_meta", attemptsTable = "question_attempts", log = console.log }) {
  ...
  try {
    ...
    log(`Class 1 Treatment additive staging complete: ${payloads.reduce((total, payload) => total + payload.questions.length, 0)} questions stored in review only.`);
    return result;
  } catch (error) {
