#!/bin/bash
# Regenerate all 4 Ontario water treatment question banks with improved WPI-style prompt
# Deletes existing questions first, then regenerates and reinserts
set -e
cd /home/ubuntu/echelon-ai-tutor
source .env 2>/dev/null || true
LOG=/tmp/regen-water-treatment.log
echo "Starting water treatment regeneration at $(date)" | tee $LOG

# ── Delete existing questions from all 4 water treatment banks ────────────────
echo "Deleting existing questions from 4 Ontario water treatment banks..." | tee -a $LOG
...
generate_and_insert "class1-water" "Water Treatment" "Class 1"
generate_and_insert "class2-water" "Water Treatment" "Class 2"
generate_and_insert "class3-water" "Water Treatment" "Class 3"
generate_and_insert "class4-water" "Water Treatment" "Class 4"

echo "All 4 water treatment banks regenerated at $(date)" | tee -a $LOG
