...
import { createBaseline, loadClass1NetworksPackage, readClass1NetworksProductionSnapshot } from "./lib/class1NetworksImporter.mjs";

if (!process.env.DATABASE_URL) throw new Error("DATABASE_URL is required for a production rollback baseline.");
const outputDirectory = process.env.CLASS1_NETWORKS_IMPORT_RELEASE_DIR ?? "/home/ubuntu/class1_networks_import_release";
...
try {
  ...
  const outputPath = path.join(outputDirectory, "class1-networks-import-baseline.json");
