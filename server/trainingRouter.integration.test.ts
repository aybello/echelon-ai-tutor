...
beforeAll(async () => {
  ...
  const [orgA] = await db.insert(organizations).values({
    name: `Training Test A ${RUN_ID}`, province: "ontario", tier: "all-access", seatsTotal: 10,
    managerEmail: MANAGER_A, termStart, termEnd, billingType: "invoice", status: "active",
  });
  orgAId = Number((orgA as any).insertId);
  const [orgB] = await db.insert(organizations).values({
    name: `Training Test B ${RUN_ID}`, province: "ontario", tier: "all-access", seatsTotal: 10,
    managerEmail: MANAGER_B, termStart, termEnd, billingType: "invoice", status: "active",
  });
...
afterAll(async () => {
  if (!db) return;
  for (const email of [OPERATOR, ZERO_ACTIVITY_OPERATOR, LEGACY_UNASSIGNED_OPERATOR]) {
...
describe("training records database integrity", () => {
  ...
  it("shows assigned operators even when they have no activity", async () => {
    if (!db) return;
  ...
  it("expires stale heartbeats without crediting additional time", async () => {
    ...
    await db.insert(learningActivitySessions).values({
      sessionKey, studentEmail: OPERATOR, orgId: orgAId, courseKey: "class1-water", activityType: "flashcards",
      activeSeconds: 30, lastSequence: 1, lastHeartbeatAt: new Date(Date.now() - 10 * 60 * 1000),
    });
    await expect(appRouter.createCaller(makeCtx(OPERATOR)).training.heartbeat({
      sessionKey, sequence: 2, activeSeconds: 30, unitsCompleted: 1,
  ...
  it("freezes the signed snapshot and denies managers from another organization", async () => {
    ...
    await db.insert(learningActivitySessions).values({
      sessionKey: firstSessionKey, studentEmail: OPERATOR, orgId: orgAId, courseKey: "class1-water",
      activityType: "process_guide", topic: "Disinfection", activeSeconds: 3600, status: "completed", completedAt: new Date(),
    });
    ...
    await db.insert(learningActivitySessions).values({
      sessionKey: crypto.randomUUID(), studentEmail: OPERATOR, orgId: orgAId, courseKey: "class1-water",
      activityType: "ai_tutor", topic: "Later activity", activeSeconds: 900, status: "completed", completedAt: new Date(),
    });
