...
describe('exact OIT batch release', () => {
  ...
  it('never writes during read-only reconciliation', async () => {
    const connection = {query:vi.fn(),rollback:vi.fn(),execute:vi.fn(async (sql:string) => sql.includes('question_bank_meta') ? [[{bankKey:'oit'}]] : [[stored()]])};
