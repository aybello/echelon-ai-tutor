...
import { loadClass1NetworksPackage } from "../scripts/lib/class1NetworksImporter.mjs";
// @ts-expect-error This standalone guarded release helper is executed as ESM by Node and Vitest.
import { planClass1NetworksRelease } from "../scripts/lib/class1NetworksRelease.mjs";
...
describe("Class 1 Distribution and Collection guarded learner-visible promotion", () => {
  it("accepts only the complete exact all-in-review package and calculates the learner-visible inventory", () => {
    ...
    expect(plan.banks.map((bank: any) => [bank.bankKey, bank.stagedCount, bank.visibleBefore, bank.expectedVisibleAfter])).toEqual([
      ["class1-water-dist", 250, 716, 966],
      ["class1-wastewater-coll", 250, 724, 974],
    ]);
  });

  it("fails closed for immutable-content drift, mixed visibility, or stale learner-visible metadata", () => {
    const fixture = stagedFixture();
    const candidate = fixture.rows.find((row) => row.bankKey === "class1-water-dist" && row.questionNum === 2001);
    ...
    mixedFixture.rows.find((row) => row.bankKey === "class1-water-dist" && row.questionNum === 2001).reviewStatus = "unreviewed";
    mixedFixture.metadata[0].totalQuestions = 717;
