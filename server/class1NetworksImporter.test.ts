...
import { CLASS1_NETWORKS_BASELINE_COUNTS, createBaseline, digest, loadClass1NetworksPackage, planClass1NetworksImport, validateBaseline } from "../scripts/lib/class1NetworksImporter.mjs";
...
describe("Class 1 Distribution and Collection guarded additive importer", () => {
  it("accepts only the checksum-validated 500-item package against the exact production baseline", () => {
    ...
    expect(plan.banks.map((bank: any) => [bank.bankKey, bank.currentCount, bank.expectedStagedCount])).toEqual([
      ["class1-water-dist", 716, 966], ["class1-wastewater-coll", 724, 974],
    ]);
  });
  ...
  it("recognizes an identical all-in-review replay while rejecting changed stored content", () => {
    ...
    const changed = rows.find((row) => row.bankKey === "class1-water-dist" && row.questionNum === 2001)!;
